...
//=====================================================================================================================
		...
		while((entry = readdir(folder))!= NULL){
			...
			if (ext && strcmp(ext, ".jack") == 0) {
				...
				while(hasMoreTokens(F_OutputFile)){
					int len = 250;
					int x;
					int y;
					char c;
					char xmlOut[250];
					advance(F_OutputFile, token, len, &stringFlag);
					x = tokenType(token, stringFlag);
					if(x == SYMBOL){
						c = symbol(token);
						fprintf(F_OutputFile, "<symbol> %c\n </symbol",c);
					}
					else if(x == KEYWORD){				
				 		y = keyword(token);
						if(y == CLASS ){
				 			fprintf(F_OutputFile, "<keyword> %s\n </keyword>",token);
				 		}else if(y == METHOD ){
				 			fprintf(F_OutputFile, "<keyword> %s\n </keyword>",token);
				 		}
						 else if(y == FUNCTION){
						...
						fprintf(F_OutputFile, "<stringConst> %s\n </stringConst>",token);
					}else if(x == INT_CONST){
						fprintf(F_OutputFile, "<intConst> %s\n </intConst>",token);
					}else if(x == IDENTIFIER){
						fprintf(F_OutputFile, "<identifier> %s\n </identifier>",token);
					}
				} 
				fclose(folderFile);
				fclose(F_OutputFile);
